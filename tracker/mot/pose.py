...
class PoseAssociationTracker(AssociationTracker):
    ...
    def prepare_obs(self, img, img0, obs):
        ...
        if len(obs) > 0:
            ...
            detections = [STrack(tlbr_to_tlwh(tlbrs), 1, f, self.buffer_size, mask, pose) \
    ...
    def update(self, img, img0, obs):
        ...
        dists = matching.fuse_motion(self.kalman_filter, dists, strack_pool, detections, 
                lambda_=self.opt.motion_lambda, gate=self.opt.motion_gated, only_position=self.opt.only_position)
